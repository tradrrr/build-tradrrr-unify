...
var Coinbase_pro = /** @class */ (function (_super) {
    ...
    function Coinbase_pro(config) {
        ...
        config = __assign({
            ws: { base_url: 'wss://ws-feed.pro.coinbase.com/' },
            http: { base_url: 'https://api.pro.coinbase.com/' },
        }, config);
        _this.merge_config(config);
        return _this;
    }
    Coinbase_pro.prototype.http_reply_from_raw = function (data, action, args) {
        if (args === void 0) { args = {}; }
        switch (action) {
            case 'ticker':
                if (!args.pair_id) {
    ...
    Coinbase_pro.prototype.http_request_to_raw = function (action, args) {
        if (args === void 0) { args = {}; }
        var r = {
            method: 'get',
        ...
        switch (action) {
            ...
            case 'ticker':
                ...
                break;
            default:
                throw new invalid_signal_1.Invalid_signal(action);
        }
        return r;
    };
    return Coinbase_pro;
